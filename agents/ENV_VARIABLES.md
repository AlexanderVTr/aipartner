...
- `CARTESIA_API_KEY` - Your Cartesia API key for text-to-speech
  - **Required:** Yes
  - **Set via:** `lk agent secrets set CARTESIA_API_KEY="your-key"`
  - **Get API key:** https://play.cartesia.ai/keys
...
- `CARTESIA_MODEL` - Cartesia TTS model to use
  - **Required:** No
  - **Default:** `"sonic-3"`
  - **Set via:** `lk agent secrets set CARTESIA_MODEL="sonic-3"`
  - **Available models:** `sonic-3`, `sonic-english`, `sonic-multilingual`

- `CARTESIA_VOICE_ID` - Cartesia voice ID
  - **Required:** No
  - **Default:** `"794f9389-aac1-45b6-b726-9d9369183238"`
  - **Set via:** `lk agent secrets set CARTESIA_VOICE_ID="your-voice-id"`
  - **Get voice IDs:** Browse voices at https://play.cartesia.ai/
...
CARTESIA_MODEL="sonic-3"  # Optional, defaults to sonic-3
CARTESIA_VOICE_ID="your-voice-id"  # Optional, has default
CARTESIA_LANGUAGE="en"  # Optional, defaults to en
```

## 🔍 Code References

- **Line 45-46:** `LIVEKIT_URL` / `NEXT_PUBLIC_LIVEKIT_URL` mapping
- **Line 107-108:** `DEEPGRAM_API_KEY`, `CARTESIA_API_KEY` (required check)
...
- **Line 197-201:** `CARTESIA_MODEL`, `CARTESIA_VOICE_ID`, `CARTESIA_LANGUAGE` (optional, have defaults)
